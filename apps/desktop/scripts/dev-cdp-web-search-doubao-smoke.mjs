/**
 * CDP E2E: MyCodexFree + gpt-5.6-luna PI Integrated Doubao web_search.
 *
 * Requires Doubao Search API key via ECO_DOUBAO_SEARCH_API_KEY, DOUBAO_SEARCH_API_KEY,
 * or apps/desktop/.local/doubao-search-api-key
 *
 *   ECO_DEV_CDP_URL=http://127.0.0.1:9334 node scripts/dev-cdp-web-search-doubao-smoke.mjs
 */
...
const MARKER = process.env.ECO_SMOKE_MARKER?.trim() || `DB${Date.now().toString(36).toUpperCase()}`;
...
function resolveDoubaoApiKey() {
  const fromEnv =
    process.env.ECO_DOUBAO_SEARCH_API_KEY?.trim() || process.env.DOUBAO_SEARCH_API_KEY?.trim();
...
function buildPrompt(marker) {
  return [
    `Eco PI Integrated Doubao web search smoke. Marker=${marker}.`,
    ...
    `DOUBAO_SEARCH_DONE:${marker}`,
    "Mention that results came from Doubao Search.",
  ].join("\n");
}
...
function step(name, ok, detail) {
  ...
  console.log(`[web-search-doubao] ${ok ? "PASS" : "FAIL"} ${name}${detail !== undefined ? `: ${detail}` : ""}`);
}
...
async function waitForDone(page, threadId, marker, timeoutMs) {
  ...
  while (Date.now() < deadline) {
    ...
    const doubaoHit = tools.find(
      (t) => t.eventType === "tool.completed" && /Doubao Search results/i.test(t.text),
    );
    const markerHit = await page.evaluate(
      ...
      { tid: threadId, marker: `DOUBAO_SEARCH_DONE:${marker}` },
    );
    if (markerHit || doubaoHit) return { markerHit, doubaoHit, tools };
  ...
  throw new Error(`Timeout waiting for Doubao web_search (${marker})`);
}
...
try {
  ...
  const doubaoApiKey = resolveDoubaoApiKey();
  if (!doubaoApiKey) {
    throw new Error(
      "Missing Doubao Search API key. Set ECO_DOUBAO_SEARCH_API_KEY or create apps/desktop/.local/doubao-search-api-key",
    );
  }
  step("resolve doubao api key", true, `len=${doubaoApiKey.length}`);
  ...
  const configured = await page.evaluate(
    ...
    { apiKey: doubaoApiKey, providerId: PROVIDER_ID, modelId: MODEL_ID },
  );

  step(
    "configure doubao integrated + disable native",
    configured.integrated.enabled &&
      configured.integrated.provider === "doubao" &&
  ...
  step("WebSearch tool completed", Boolean(outcome.doubaoHit || outcome.tools?.some((t) => /WebSearch/i.test(t.text))));
  step("assistant marker or doubao result", Boolean(outcome.markerHit || outcome.doubaoHit));

  await page.screenshot({ path: ".smoke-artifacts/cdp-web-search-doubao-smoke.png", fullPage: false });
  step("screenshot", true, ".smoke-artifacts/cdp-web-search-doubao-smoke.png");
