...
export function isLiveFeedSkeletonAgent(agent: Pick<ThreadRunProjectionAgent, "status">): boolean {
  return agent.status === "active" || agent.status === "launching";
}

export function isAgentScopedFeedTimelineItem(item: Pick<ThreadRunProjectionTimelineItem, "scope">): boolean {
...
/** Items that can become a segment final after a late user boundary is replayed. */
export function isSkeletonSegmentFinalItem(item: ThreadRunProjectionTimelineItem): boolean {
...
function lastUserBoundaryForItem(
  ...
  for (const boundary of boundaries) {
    // Provider rows can receive their durable sequence after the user prompt
    // was observed (especially after replay/migration). Use the same wall-clock
    // turn boundary for tools, errors and narrative rows so incremental staging
    // and a full rebuild classify the event identically.
