...
export function computePromptCacheHitRatio(usage: RequestCacheUsage): number | null {
  ...
  const billedPromptTokens = inputTokens + cacheReadTokens;
...
export function detectPromptCacheHitDrop(
  ...
  return {
    ...
    cacheReadTokens: Math.max(0, current.cacheReadTokens),
...
export function formatPromptCacheHitDropMessage(detection: CacheHitDropDetection): string {
  ...
  return `Prompt cache 命中率从 ${previousPct}% 降至 ${currentPct}%（↓${dropPp}pp），可能由 cache break 引起。本轮 cache_read ${detection.cacheReadTokens.toLocaleString("en-US")} / input ${detection.inputTokens.toLocaleString("en-US")}`;
}
