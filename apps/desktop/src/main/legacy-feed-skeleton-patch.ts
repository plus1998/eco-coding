...
import {
  ...
  isSkeletonSegmentFinalItem,
  isSkeletonUserPromptItem,
  selectSkeletonTimelineItems,
} from "../shared/conversation-v2-projection-skeleton";
import type { ThreadRunEvent, ThreadRunProjectionAgent } from "../shared/ipc";
import { eventToTimelineItem } from "./conversation-v2-runtime-projection";
import { trimTimelineItemForFeed } from "./legacy-feed-replay-projection";
import {
  FEED_SKELETON_RULES_VERSION,
  type FeedSkeletonPatchState,
  type ThreadFeedSkeletonRecord,
} from "./legacy-feed-skeleton-store";
import { type FeedTimelineStageState, stageFeedTimelineEvent } from "./legacy-feed-skeleton-timeline-items";
...
export function createFeedSkeletonPatchState(
  ...
  const seeded = events.reduce<FeedTimelineStageState>(
    (state, event) => stageFeedTimelineEvent(state, event),
    { items: [], finalizedSdkBlocks: [] },
  );
  const hasSeedEvents = events.length > 0;
  return {
    // Keep the complete staged prefix when a compatibility rebuild has not seen a
    // user boundary yet. A legacy replay may deliver prompt rows after provider
    // rows whose observedAt is inside that turn; selecting only the current final
    // would make a later prompt unable to split the already-dropped candidates.
    trackedItems: (hasSeedEvents ? seeded.items : snapshot.timeline).map((item) => ({ ...item })),
    ...
    finalizedSdkBlocks: hasSeedEvents
      ? [...seeded.finalizedSdkBlocks]
      : collectSettledSdkMessageBlocks(events),
    rulesVersion: FEED_SKELETON_RULES_VERSION,
    retainCandidateFinals: hasSeedEvents && !snapshot.timeline.some(isSkeletonUserPromptItem),
  };
}
...
export function patchThreadFeedSkeletonFromEvent(
  ...
  const retainCandidateFinals = record.patchState.retainCandidateFinals === true;
  return {
    historyRevision: context.historyRevision,
    maxEventSequence: context.maxEventSequence,
 ...
 * boundaries, and the whole trail of attempts that are still running. During a compatibility
 * replay, retainCandidateFinals also keeps candidate segment finals until a late user boundary
 * is observed; migrated provider rows can receive durable sequence values after that boundary.
 * Also used when only attempt states changed (a run finished without its own event), where
 * re-selecting the tracked items is enough and no event log read is required.
 */
export function reconcileFeedSkeletonTrackedItems(
  items: readonly ThreadRunProjectionTimelineItem[],
  attempts: readonly ThreadRunProjectionAttempt[],
  options: { retainCandidateFinals?: boolean } = {},
): ThreadRunProjectionTimelineItem[] {
  ...
  for (const item of items) {
    ...
    if (options.retainCandidateFinals && isSkeletonSegmentFinalItem(item)) {
      kept.set(item.id, item);
      continue;
    }
