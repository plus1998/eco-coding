...
一条命令：编译 amd64 二进制 → 上传 → systemd 重启。Mongo/Redis 读 `apps/server/.env`，**默认不启动 Docker**。

```sh
bun run deploy:server -- root@192.168.31.204      # 部署 / 更新
bun run deploy:server -- root@192.168.31.204 -b    # 强制重新编译
```

`.env` 要点：`ECO_SERVER_HOST=0.0.0.0`，Mongo/Redis 填实际地址。

| 特殊场景 | 加参数 |
|----------|--------|
| 没有 Mongo/Redis，用容器启动 | `--with-deps` |
| 旧 Docker 全容器模式 | `--docker` |

仅编译不上传：`bun run build:server`

## 1Panel / Docker Compose（可选）
...
```sh
bun run deploy:server -- root@host --docker -b
```
...
1. 本地：`bun run deploy:server -- root@host -b`
2. 或在服务器：`cd /opt/eco/server && docker compose up -d --build`（仅 `--docker` 模式）
