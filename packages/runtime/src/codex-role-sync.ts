...
export function buildCodexRoleToml(input: {
  ...
  const isBuiltInOpenAi = input.providerId === "openai";
  const modelValue = isBuiltInOpenAi ? input.modelId : buildCodexGatewayModelAlias(input.providerId, input.modelId, input.apiCompat);
  const lines = [
    ...
    `model = ${tomlString(modelValue)}`,
    ...(isBuiltInOpenAi ? [] : [`model_provider = ${tomlString(buildCodexModelProviderSlug(input.providerId))}`]),
