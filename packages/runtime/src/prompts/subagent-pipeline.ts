...
export function buildEcoPlanHarnessAdapter(availability: SubagentAvailability): string {
  ...
  return [
    ...
    "- The `ExitPlanMode` tool input must include the complete Markdown plan in the `plan` field. Do not call `ExitPlanMode` with `{}` or only `allowedPrompts`.",
    ...
    "- If the user revises the spec after a prior submitted plan, the next `ExitPlanMode` plan must be a complete replacement in the `plan` field, not a partial diff.",
    "",
