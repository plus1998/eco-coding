...
export function buildEcoPlanHarnessAdapter(availability: SubagentAvailability): string {
  ...
  return [
    ...
    "- Final plan submission: **`FinalizePlan`** with `{ analysis, plan }` strings.",
    exploreLine,
    ...
    "2. Required: submit decision-complete plan via `FinalizePlan`.",
    "   - `FinalizePlan.analysis`: complete analysis summary string.",
    "   - `FinalizePlan.plan`: complete implementation plan string (Summary/Key Changes/Test Plan/Assumptions).",
    ...
    "",
    turn1Explore,
    "- **Do not finalize**: MUST NOT call `FinalizePlan` on turn 1.",
    ...
    "- Still MUST NOT call `FinalizePlan` until decision-complete per Codex Finalization rule.",
    "",
    "### Final turn only",
    "",
    "- Call `FinalizePlan` once spec is decision-complete (unanswered preference questions use recommended defaults recorded under Assumptions).",
    "",
    ...
    "the next `FinalizePlan` payload MUST be a **complete replacement** — same rule as Codex `<proposed_plan>` revisions, not a partial diff.",
  ].join("\n");
}
