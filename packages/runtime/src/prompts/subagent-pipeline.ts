...
export function buildEcoPlanHarnessAdapter(availability: SubagentAvailability): string {
  ...
  return [
    ...
    exploreLine,
    "- External facts (official docs, API versions, third-party behavior, current best practices not in the worktree): use **`WebSearch`**; open a specific URL with **`WebFetch`** after repo exploration — do not skip the worktree for in-repo questions.",
    ...
    "",
    exploreFirstRule,
    "- **External lookup when needed**: after repo exploration, use `WebSearch` / `WebFetch` only for facts outside the worktree (not for code or config discoverable locally).",
