...
describe("custom freeform tool protocol", () => {
  ...
  test("Chat non-stream response restores custom_tool_call", () => {
    ...
    expect(
      restored.output?.some((item) => item.type === "custom_tool_call" && item.name === "apply_patch"),
    ).toBe(true);
  ...
  test("Chat stream restores custom_tool_call_input.done from fragmented arguments", () => {
    ...
    expect(
      events.some(
        (event) => event.type === "response.output_item.done" && event.item?.type === "custom_tool_call",
      ),
    ).toBe(true);
  ...
  test('Anthropic request converts custom tools without type:"custom"', () => {
    ...
    expect(anthropic.tools?.some((tool) => (tool as { type?: string }).type === "custom")).toBe(false);
    const applyTool = anthropic.tools?.find((tool) => tool.name === "apply_patch");
    expect(applyTool?.input_schema).toMatchObject({
      type: "object",
      required: [CUSTOM_TOOL_INPUT_FIELD],
    });
    const assistantContent = anthropic.messages.find((message) => message.role === "assistant")?.content;
  ...
  test("Anthropic non-stream and stream restore custom_tool_call", () => {
    ...
    expect(streamEvents.some((event) => event.type === "response.custom_tool_call_input.done")).toBe(true);
  ...
  test("ordinary function tools still round-trip as function_call", () => {
    ...
    const restored = chatCompletionsResponseToResponses(chatResp, "m", buildCodexToolContextFromRequest(req));
