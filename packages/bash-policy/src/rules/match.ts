...
import type { BashPolicyRules, CommandDenyRule, CommandSegment, DenyMatch, ShellAst } from "../types";

export function matchDeny(ast: ShellAst, rules: BashPolicyRules): DenyMatch | null {
...
function matchesDenyRule(segment: CommandSegment, rule: CommandDenyRule): boolean {
...
function formatDenyTarget(segment: CommandSegment, rule: CommandDenyRule): string {
...
export function scoreMatchesRule(segment: CommandSegment, program: string, rules: BashPolicyRules): number {
...
function matchesScoreWhen(
  segment: CommandSegment,
  rule: { flagsContain?: string[]; argsContain?: string[]; targetsContain?: string[] },
): boolean {
...
export function scorePipeline(ast: ShellAst, rules: BashPolicyRules): number {
  let maxScore = 0;
...
function matchesPipelinePattern(programs: string[], pattern: string[]): boolean {
  if (pattern.length < 2) {
    return false;
  }
