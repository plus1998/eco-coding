...
1. 首台设备：生成 `vault_key`，加密 API Key 上传（同步一次即可）。  
2. 新设备 Auth 登录后发起 `vault_claim`（**不要求**对方此刻在线；请求写入服务器，默认约 24h TTL）。  
3. 任意已持有 `vault_key` 的设备在打开 Eco 后发现 pending claim，用户批准并显示 6 位码；新设备输入。  
4. 6 位仅绑定 claim；`vault_key` 用短时会话密钥（ECDH 等）加密下发后销毁临时材料。  
5. 账号下尚无任何 vault-synced 设备 → 不能走 claim（应在首台先同步）；有已同步设备但暂时离线 → 可先挂请求，等对方开机批准。
