...
本文档记录 Eco 集成 Claude Agent SDK 时的工具模型、常见踩坑，以及 Eco 自己的映射方式。目的是避免把「工具可见」「自动批准」「orchestration 禁止」混为一谈。
...
Eco orchestration 的工具策略只应通过显式规则表达：
...
Bash 与写工具通过 SDK `disallowedTools` 从模型上下文移除；读工具（Read / Grep / Glob）保持可见。Eco PreToolUse hook 与 orchestration `disallowed`（经物化）作为第二层执行校验。
...
| `bashReviewMode: allow_all` = **开启** Bash | ❌ 只是**跳过 Bash 审批**；orchestration / disallow 仍可禁用 |
...
1. Bash 是否在 SDK `disallowedTools` 或 Eco orchestration `disallowed` 里
...
```
orchestration 结构化开关 (bash / filesystem / network)
...
| `mergeSdkDisallowedTools()` | 合并 orchestration + 阶段 denylist，传给 SDK |
...
## 5. Eco orchestration 工具策略映射

orchestration 表单的「工具能力」最终翻译为 `EcoToolPolicy`：
...
4. **Eco PreToolUse**：orchestration `disallowed`、filesystem、network、bash 策略是否拒绝？
5. **阶段**：当前是 `planning` / `ask` / `execution`？Plan 与 Ask 阶段 Bash 应在 SDK `disallowedTools` 与物化后的 orchestration `disallowed` 中。

错误信息对照：

| 消息 | 通常来源 |
|------|----------|
| `Tool "Bash" is disallowed for main.` | Eco orchestration 物化后的 `disallowed`（显式禁止，符合预期） |
...
| orchestration → SDK policy | `packages/runtime/src/agent-orchestration.ts` |
...
| orchestration 工具能力表单 | `apps/desktop/src/renderer/tool-capability-groups.ts` |
