...
- `bun test`: 通过，`817 pass / 14 skip / 0 fail`
...
- Agent lifecycle 仍为 shadow 写入，不驱动 UI、不替代旧 activity 展示；`activeRuns` 已拆成 runtime state / billing state，run attempt lifecycle/retry settlement 编排已抽到 `thread-run-attempt`，run outcome 判定已抽到 `thread-run-outcome`，run decision 公共副作用分派已抽到 `thread-run-decision-effects`，run finally cleanup 已抽到 `thread-run-cleanup`，thread runtime route/proxy attempt 生命周期已抽到 `thread-runtime-routes` / `thread-runtime-proxy-attempt`，SDK event stream 消费已抽到 `sdk-run-event-loop`，SDK driver input 构造已抽到 `sdk-run-input`，SubAgent 父 tool use 队列已抽到 `subagent-tool-use-index`，active/stopped agent 解析规则已抽到 `subagent-agent-resolver`，SubAgent metrics entry/status/context 状态已抽到 `subagent-metrics-state`，legacy usage fallback 已抽到 `subagent-legacy-usage`，SubAgent metrics diagnostics 已抽到 `subagent-metrics-diagnostics`，SubAgent metrics registry persistence facade 已抽到 `subagent-metrics-registry-persistence`，SubAgent metrics record target 判定已抽到 `subagent-metrics-record-target`；`SubagentMetricsRegistry` 仍保留 tool use linkage 和 context/legacy fallback 编排。
...
1. 继续收敛 `SubagentMetricsRegistry` 的 tool use linkage 编排，优先把 parent tool use 链接决策从 Registry 主类移出。
2. 梳理 legacy `recordSdkUsage` fallback 的真实调用面和 projection gate 条件，准备可审计删除路径。
