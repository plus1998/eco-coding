...
1. 继续收缩 `SubagentMetricsRegistry` 的 persistence facade，降低 restore/persist 与 registry 状态机的耦合。
2. 继续拆更细的 run branch side-effects，把取消、失败、完成、awaiting_plan 的副作用调用顺序逐步收口到可测试 helper。
