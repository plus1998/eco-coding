...
- TODO list IPC 回填已抽到 `thread-todo-list-runtime`；运行中 SDK TaskCreate/TaskUpdate/todo.updated tracker 仍由 `sdk-task-tracker` 和 `sdk-task-run-hooks` 驱动，主进程只负责装配 store 和 event emitter。
