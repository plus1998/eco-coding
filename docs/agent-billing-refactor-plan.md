...
- `bun test`: 通过，`818 pass / 14 skip / 0 fail`
...
2. 收缩 `SubagentMetricsRegistry` 对 legacy usage tracker 的直接编排，优先把 fallback record 输入构造移到独立 helper。
3. 梳理 `index.ts` 剩余 pending plan/todo UI glue，寻找可测试的小型副作用 helper。
4. 在更多真实路径对账稳定后，逐步删除 legacy synthetic SDK primary 兼容写入与 `recordSdkUsage` fallback。
