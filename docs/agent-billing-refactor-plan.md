...
- Pending plan IPC 读取视图已抽到 `thread-pending-plan-view`，审批启动门禁已抽到 `thread-plan-approval-runtime`；approve handler 仍持有状态更新和启动执行副作用，dismiss pending plan 仍在主进程持有运行时取消与 worktree 清理副作用，后续需要以完整行为切点继续拆分。
...
2. 梳理 `index.ts` 剩余 pending plan approve/dismiss/continuation 副作用，寻找可测试的完整运行时切点。
