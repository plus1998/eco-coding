...
第五十一批 Thread run cleanup 已完成：

- 新增 `thread-run-cleanup`，把 run finally 里的 clarification cancel、SDK stream reset、usage flush、active run finish、post-run context refresh、running fallback idle 收口到显式依赖的 cleanup helper。
- question、autonomous execution、planning、approval continuation、manual execution 和普通 continuation 的 finally 清理改为调用 `finalizeMainThreadRunCleanup`。
- helper 保持旧路径差异：approval continuation 不做 idle fallback，manual execution 不额外取消 clarification，特殊 autonomous continuation 分支仍保留原有 finish/context refresh 行为，避免扩大行为面。
- cleanup helper 允许 `worktreePath` 为空，保持旧的 `afterRunContextRefresh(threadId, undefined)` 语义。
- 新增测试覆盖：清理顺序、flush await 顺序、running fallback idle、非 running 不更新 idle、可跳过 clarification cancel 和 idle fallback。

第五十二批 SubAgent metrics diagnostics facade 已完成：

- 新增 `subagent-metrics-diagnostics`，把 SubAgent lifecycle、task tool、resolve miss、usage dedupe 的诊断 topic 和字段格式收口到独立门面。
- `SubagentMetricsRegistry` 不再直接依赖 `eco-diag-log` 的短 ID 和 payload 拼装，改为通过 `SubagentMetricsDiagnosticsPort` 注入诊断依赖。
- Registry 的计费、状态、持久化语义保持不变；本批只移动诊断职责边界，避免日志字段拼装继续散落在 metrics 状态机里。
- 新增测试覆盖：diagnostics payload 格式、短 thread/agent/tool id 规则、Registry 通过注入 port 发出 lifecycle/task tool 事件。

第五十三批 SubAgent metrics registry persistence facade 已完成：

- 新增 `subagent-metrics-registry-persistence`，把 store row 恢复、entry upsert、clear thread 委托和 legacy usage contribution 构造收口到 registry 专用 persistence facade。
- `SubagentMetricsRegistry` 构造保持兼容，外部仍传现有 `SubagentMetricsPersistenceStore`；内部改为依赖 `SubagentMetricsRegistryPersistence`，便于后续把数据库细节继续移出状态机。
- `restoreFromStore` 现在只负责把已恢复 entry 放回内存状态、恢复 legacy usage 去重键，不再直接读取 persistence row 或手写 mapper。
- 新增测试覆盖：facade restore、legacy usage contribution、persist mapper 委托、clear 委托。

第五十四批 Thread run decision effects 已完成：

- 新增 `thread-run-decision-effects`，把 run outcome decision 到通用副作用的分派规则收口到可测试 helper。
- helper 内建 `awaiting_plan`、`idle`、带 message 的 `completed` 状态更新；`cancelled` / `failed` 仍要求调用方显式提供回调，避免隐藏 worktree rollback、clarification、session cleanup 等路径特有动作。
- question、autonomous planning/execution、planning 三条主入口改为通过 `applyMainThreadRunDecisionEffects` 处理 cancelled、failed、awaiting_plan、idle/completed 的公共分支。
- 本批不触碰 manual execution 和 continuation 的 todo stop / restore 顺序，避免把复杂副作用一次性搬迁造成行为漂移。
- 新增测试覆盖：cancelled/failed 回调分派、默认状态更新、成功回调覆盖、无可用 handler 时返回 false。

第五十五批 Thread run decision effects 复杂分支接入已完成：

- approval execution、manual execution、已有 worktree continuation、普通 continuation 的 run outcome 分支都改为通过 `applyMainThreadRunDecisionEffects` 分派。
- 保留原有分支顺序：manual execution 的 todo stop 仍先于 cancel/restore，失败仍走 `restoreAfterExecutionFailure`；continuation 的 session cleanup、todo stop、question title summary 和 execution completion 顺序保持不变。
- continuation 的 `awaiting_plan` / `idle` 状态更新使用 helper 默认状态更新；execution/question completed 使用显式 `onCompleted`，避免把不同 mode 的成功副作用混在 helper 内部。
- 新增测试覆盖：helper 会等待 async handler 完成后再返回，支撑 worktree cancel/restore 这类异步副作用调用。

第五十六批 SubAgent metrics record target 已完成：

- 新增 `subagent-metrics-record-target`，把 SubAgent metrics 写入前的 `resolveAgentId -> roleForAgentId -> isSubagentBillingRole` 判定收口到可测试门面。
- `SubagentMetricsRegistry.recordContextObservation` 和 legacy `recordSdkUsage` 复用同一个 record target 解析结果，避免 context 与旧账单 fallback 各自维护一套归因 guard。
- helper 只接受最小 resolver 接口，不读取 Registry 内部状态；显式 agent、parent tool use、非 SubAgent registry role 拒绝等规则由单测固定。
- 本批不改变 legacy fallback、tool use linkage、持久化、账本投影或 UI 语义，只移除重复目标解析逻辑。
- 新增测试覆盖：成功解析、显式 agent/parent tool use 参数透传、未解析 agent 不查 role、非 SubAgent role 拒绝。

第五十七批 SubAgent parent tool use 原子绑定已完成：

- `SubagentToolUseIndex` 新增 `linkNextPendingForRole`，把按 role 消费 pending tool use 并绑定到 agent 的动作收口为单个原子语义方法。
- `SubagentMetricsRegistry.onSubagentStart` 不再手写 `consumeForRole -> link` 组合，只接收 tool use index 返回的 mapped count 用于 lifecycle diagnostics。
- 原有 `consumeForRole` / `link` API 保留，避免影响既有测试和外部兼容；新入口用于减少 Registry 对队列内部编排顺序的了解。
- 新增测试覆盖：按 role 原子绑定、resolve 映射、pending 移除、无 pending 时保持 mapped count 不变。

第五十八批 SubAgent legacy fallback target record 已完成：

- `SubagentLegacyUsageTracker` 新增 `recordForTarget`，把 resolved target 与 legacy usage observation 拼成完整 record 的责任收口到旧 fallback tracker。
- `SubagentMetricsRegistry.recordSdkUsage` 现在只负责解析可记录 target、调用 fallback tracker、持久化非 dedupe 结果和发送 dedupe diagnostics，不再手写 legacy record 字段。
- 原 `record` API 保留，现有幂等、restore contribution 和测试入口不变，避免扩大 legacy fallback 的迁移风险。
- 新增测试覆盖：resolved target observation 记录 agent/role、usage、cost、model 和更新时间。

第五十九批 SubAgent legacy metrics fallback gate 已完成：

- 新增 `subagent-legacy-metrics-fallback`，把 legacy `recordSdkUsage` fallback 的触发条件命名化为可测试决策。
...
第六十一批 Thread plan ready effects 已完成：

- 新增 `thread-plan-ready-effects`，把 SDK `plan.ready` 后的 pending plan 持久化、`thread.awaiting_plan` 事件 payload、标题摘要触发收口到可测试 helper。
- 初次 autonomous、manual planning、continuation planning 三条路径统一调用 `captureThreadPlanReady`，保留原有两种用户可见提示文案。
- helper 只通过显式 effects 接口访问 store/event/title 副作用，`index.ts` 只保留 Electron main-process adapter，减少重复 UI glue。
- 新增测试覆盖：pending plan 字段、awaiting_plan payload、标题摘要上下文、调用方文案不污染持久化计划。

第六十二批 SDK task run hooks 已完成：

- 新增 `sdk-task-run-hooks`，把 SDK task tracker 的 hook extras、top-level pending todo hint、Stop hook 去重和 owner status 管理收口到可测试 helper。
- 批准后执行与 continuation execution 统一使用 `createSdkTaskRunHooks`，删除局部 `stopStatusRef` / `stopTodosHandled` 组合，避免 SDK Stop 与 run decision 分支重复结算 running todos。
- `createSdkDriver` 和 `buildSdkHookContextExtras` 改为接受 desktop 专用 `SdkRunHookContextExtras`，不再把 `peekPendingCoderTodoId` 伪装成 runtime 公共 `EcoHookContext` 字段。
- 新增测试覆盖：pending todo hint 透传、Stop 只处理一次、SDK Stop 不覆盖 run owner 决定的最终 todo 状态。

第六十三批 SubAgent legacy fallback effects 已完成：

- 新增 `subagent-legacy-metrics-fallback-effects`，把 legacy `recordSdkUsage` fallback 的“判定、记录、强制回 legacy billing snapshot”收口到一个 effects helper。
- `applySingleUsageBillingEffects` 和 `applySdkRunBillingEffects` 不再各自手写 `resolveSubagentLegacyMetricsFallback -> recordSdkUsage -> resolveBillingSnapshot(useLedgerProjection:false)`。
- helper 使用显式 `recordSdkUsage` / `resolveBillingSnapshot` 服务接口，后续删除旧 fallback 或改成审计日志时只需替换一个入口。
- 新增测试覆盖：ledger projection 可用时跳过、projection 缺失时写 records 并保留 plannerModelLabel 后强制选择 legacy snapshot。

第六十四批 Synthetic SDK primary fill gate 已完成：

- 新增 `resolveSyntheticSdkPrimaryFill`，把 legacy synthetic SDK primary 兼容写入条件命名化为带 reason 的可审计决策。
- `recordLegacySingleUsageBilling` 现在返回 `syntheticSdkPrimaryDecision`，后续删除 legacy primary fill 或改为诊断日志时有单一切点。
- 决策 reason 覆盖 `not_requested`、`non_subagent_role`、`missing_agent`、`already_seen`、`subagent_compatibility`。
- 本批不改变旧 accumulator 写入结果，只让兼容写入是否发生可追踪。

第六十五批 SubAgent billing metrics effects 已完成：

- 新增 `subagent-billing-metrics-effects`，把 SubAgent context observation 与 legacy metrics fallback record 的字段构造收口到纯函数。
- `applySingleUsageBillingEffects` 与 `applySdkRunBillingEffects` 不再各自手写 `agentId/role/context/model/requestKey/billing` 拼装，避免 single usage 和 SDK final 两条路径后续分叉。
- 新 helper 明确只接受 SubAgent role 和有效 agentId，context occupancy 优先来自 instance snapshot，缺失时才按 input + cache tokens 估算。
- 新增测试覆盖：非 SubAgent / 缺 agent 跳过、实例快照优先、fallback occupancy 规则、context observation 与 legacy record 字段对齐。

第六十六批 SDK usage billing dispatch 已完成：

- 新增 `sdk-usage-billing-dispatch`，把 SDK usage 解析结果到 assistant fallback、stream partial、SDK final 三条计费处理链的分派收口到可测试模块。
- `index.ts` 保留 SDK payload 解析、active run 状态读取和诊断日志实现，分派模块负责 `trackUsageUpdate`、错误转审计输出、以及 stream/final resolution logging 的调用时机。
- 新增测试覆盖：none / assistant_ignored 不派发、assistant fallback 不写 resolution log、stream partial 与 SDK final 会 track 并记录 resolution、SDK final 失败会转成 `[eco] SDK run billing failed` 审计错误且 tracked promise resolve。
- 本批不改变 SDK usage attribution、request key、ledger event 或 UI payload，只减少 main process 中的三条计费分支重复 glue。

第六十七批 SDK usage recorded event handler 已完成：

- 新增 `sdk-usage-recorded-event-handler`，把 SDK usage recorded event 的 context 事件短路、run attempt/planner agent/authoritative observations 读取、usage billing resolution、dispatch 调用收口到可测试 handler。
- `index.ts` 只保留 handler service adapter：context lifecycle、Agent lifecycle、active run billing state、SubAgent resolver、dispatch services。
- 新增测试覆盖：context event 不读取 usage 状态、SDK final resolution 带上 runAttempt/plannerAgent/subagentAgent、authoritative observation 会抑制 assistant fallback。
- 本批不改变 SDK event 解析规则、dispatch 错误处理、诊断日志内容或账本写入，只把 main process 中的隐式前置依赖显式化。

第六十八批 SubAgent legacy fallback 调用面收口已完成：

- 新增 `usage-subagent-legacy-fallback`，把 single usage 与 SDK final 两条 effects 中的 legacy `recordSdkUsage` record 构造和 fallback gate application 收口到一个可测试入口。
- `usage-billing-effects` 不再手写 legacy SubAgent records，只负责解析 SubAgent context、生成 legacy billing snapshot、注入 record/selection services。
- 新增测试覆盖：single usage 只生成一条 legacy record、缺少 SubAgent context 时不触发旧 metrics、SDK final 按每个 model row 生成 legacy records。
- 本批不改变 fallback 触发条件、ledger projection gate、legacy billing 强制回退或 SubAgent metrics 累计语义，只让后续删除旧 fallback 的真实调用面更明确。

第六十九批 Thread todo list runtime 已完成：

- 新增 `thread-todo-list-runtime`，把 `thread:todo-list` IPC 的“已有 TODO 优先、无 TODO 时从 planner/architect 活动回填、回填成功后持久化”流程收口到可测试 helper。
- `index.ts` 的 IPC handler 只保留 threadId 校验和 ConversationStore adapter，不再直接调用 coder task parser/merge 逻辑。
- 新增测试覆盖：已有 TODO 时不读取 activity、无可提取任务时不写 store、从 planner activity 回填并保存 TODO。
- 本批不改变 Coder Task 提取规则、TODO id/status/position 语义、SDK task tracker 或运行中 todo.updated 处理，只降低主进程 IPC glue 密度。

第七十批 Thread SDK task runtime 已完成：

- 新增 `thread-sdk-task-runtime`，把运行中 SDK Task tracker 与 task run hooks 的装配、`todo.updated` payload guard 和 progress event 分派收口到一个 runtime helper。
- `index.ts` 的 plan approval execution 与 continuation execution 分支只保留 store/event emitter adapter，不再直接创建 `createSdkTaskTracker` / `createSdkTaskRunHooks`。
- 新增测试覆盖：hook extras 通过 thread store/emitter 更新 TODO、非 `todo.updated` 或无 `sdkKind` payload 会跳过、合法 `todo.updated` 会推进 TODO 状态。
- 本批不改变 TaskCreate/TaskUpdate/TodoWrite/SubagentStart/SubagentStop 语义，也不改变 Stop hook 的去重和状态归属规则。

第七十一批 Thread pending plan view 已完成：

- 新增 `thread-pending-plan-view`，把 `thread:get-pending-plan` 的内部 pending plan record 到 IPC view 的字段投影收口到纯函数。
- `index.ts` 的 IPC handler 只保留 threadId 校验和 ConversationStore 读取，不再直接复制 pending plan 字段或接触内部 `routesJson` 泄漏规则。
- 新增测试覆盖：无 pending plan 返回 undefined、返回 IPC view 时剥离内部 routing 字段。
- 本批不改变 approve/dismiss pending plan、run continuation、worktree cleanup 或 pending plan 持久化语义，只让读取视图边界可测试。

第七十二批 Thread plan approval runtime gate 已完成：

- 新增 `thread-plan-approval-runtime`，把 `thread:approve-plan` 的线程状态、active run、pending plan、空计划、runtime config 与 manual/autonomous 启动分流门禁收口到可测试 helper。
- `index.ts` 的 approve IPC handler 只保留 payload 解析、running 状态更新、按 `launchMode` 启动执行函数和返回线程快照。
- 新增测试覆盖：thread 缺失、非 awaiting_plan、已有 active run、pending plan 缺失、空计划、runtime config 失败、manual 与 autonomous launch context。
- 本批不改变 plan approval 后的执行函数、worktree 隔离、账单归因、pending plan 清理或 approved plan snapshot 写入，只让审批启动前置条件成为统一领域边界。

第七十三批 Thread pending plan dismissal decision 已完成：

- 新增 `thread-pending-plan-dismissal`，把 dismiss/cancel pending plan 时的 workspace 来源选择、pending worktree hint 传递、隔离 worktree 取消或 idle 回退决策收口到纯函数。
- `dismissPendingPlan` 保留 abort、clear pending plan、`handleRunCancelled`、idle 状态更新和事件发送副作用，只按 helper 返回的 action 分派。
- 新增测试覆盖：pending workspace 优先于 thread workspace、缺 pending 时回退 thread workspace、无 workspace 直接 idle、非隔离 worktree plan 直接 idle。
- 本批不改变 active run abort、pending plan 清除顺序、worktree cancel finalizer、取消 disposition、账单结算或 Agent lifecycle settlement，只让 dismiss 前的状态决策可审计。

第七十四批 SubAgent legacy fallback record count 已完成：

- `AppliedSubagentLegacyMetricsFallback` 新增 `recordCount`，把 legacy `recordSdkUsage` fallback 是否触发与实际写入几条旧 metrics record 区分开。
- `usage-subagent-legacy-fallback` 的 single usage 与 SDK final 调用面直接透出该计数，后续删除或降级 legacy fallback 时可按真实写入量审计。
- 新增测试覆盖：ledger projection available / missing context 时计数为 0、single usage 写入 1 条、SDK final 按 model rows 写入 2 条、projection unavailable 但 records 为空时 reason 保留且 recordCount 为 0。
- 本批不改变 fallback gate、`recordSdkUsage` 入参、legacy billing snapshot 强制选择或账单展示行为，只增强返回结果的可追踪性。

当前边界：

- Agent lifecycle 仍为 shadow 写入，不驱动 UI、不替代旧 activity 展示；`activeRuns` 已拆成 runtime state / billing state，run attempt lifecycle/retry settlement 编排已抽到 `thread-run-attempt`，run outcome 判定已抽到 `thread-run-outcome`，run decision 公共副作用分派已抽到 `thread-run-decision-effects`，plan.ready 公共副作用已抽到 `thread-plan-ready-effects`，plan approval runtime gate 已抽到 `thread-plan-approval-runtime`，pending plan dismissal decision 已抽到 `thread-pending-plan-dismissal`，SDK task run hook extras 与 Stop 去重已抽到 `sdk-task-run-hooks`，run finally cleanup 已抽到 `thread-run-cleanup`，thread runtime route/proxy attempt 生命周期已抽到 `thread-runtime-routes` / `thread-runtime-proxy-attempt`，SDK event stream 消费已抽到 `sdk-run-event-loop`，SDK usage recorded event handler/dispatch 已抽到 `sdk-usage-recorded-event-handler` / `sdk-usage-billing-dispatch`，SDK driver input 构造已抽到 `sdk-run-input`，SubAgent 父 tool use 队列与 start 原子绑定已抽到 `subagent-tool-use-index`，active/stopped agent 解析规则已抽到 `subagent-agent-resolver`，SubAgent metrics entry/status/context 状态已抽到 `subagent-metrics-state`，SubAgent billing metrics 字段构造已抽到 `subagent-billing-metrics-effects`，legacy usage fallback 与 target observation record 已抽到 `subagent-legacy-usage`，legacy metrics fallback gate/effects/call surface 已抽到 `subagent-legacy-metrics-fallback` / `subagent-legacy-metrics-fallback-effects` / `usage-subagent-legacy-fallback`，SubAgent metrics diagnostics 已抽到 `subagent-metrics-diagnostics`，SubAgent metrics registry persistence facade 已抽到 `subagent-metrics-registry-persistence`，SubAgent metrics record target 判定已抽到 `subagent-metrics-record-target`，pending plan IPC view 已抽到 `thread-pending-plan-view`，synthetic SDK primary 兼容写入门禁已抽到 `resolveSyntheticSdkPrimaryFill`；`SubagentMetricsRegistry` 仍保留 context/legacy fallback 调用编排和对 tool use index 的生命周期接入。
...
- Legacy fallback 返回结果已带 `recordCount` 审计信号；该计数只用于判断旧 metrics 实际写入量，不作为账单来源。
...
- `processUsageBilling`、`processSdkRunBilling`、`processSdkStreamPartialUsage` 的用量副作用已抽到 `usage-billing-effects`；旧 accumulator 兼容写入已抽到 `usage-legacy-billing`，其中 synthetic SDK primary fill gate 已具备可审计 reason；context update 参数构造和 usage context service 已抽到 `usage-context-effects`；SubAgent context observation 与旧账单累计 fallback 已在 `SubagentMetricsRegistry` 中分离，字段构造统一由 `subagent-billing-metrics-effects` 提供，legacy fallback 调用面已由 `usage-subagent-legacy-fallback` 统一承接；SubAgent metrics IPC summary 已通过 coordinator projection 输出；SubAgent metrics persistence port、row/entry mapper、legacy usage key 已收缩到 `subagent-metrics-persistence`；SubAgent 父 tool use 队列与 start 原子绑定已抽到 `subagent-tool-use-index`；SubAgent agent 解析规则已抽到 `subagent-agent-resolver`；SubAgent metrics entry/status/context 状态已抽到 `subagent-metrics-state`；SubAgent legacy usage fallback 与 target observation record 已抽到 `subagent-legacy-usage`；SubAgent legacy metrics fallback gate/effects 已抽到 `subagent-legacy-metrics-fallback` / `subagent-legacy-metrics-fallback-effects`；SubAgent metrics diagnostics 已抽到 `subagent-metrics-diagnostics`；SubAgent metrics registry persistence facade 已抽到 `subagent-metrics-registry-persistence`；SubAgent metrics record target 判定已抽到 `subagent-metrics-record-target`；线程 metrics restore/persist/flush 已抽到 `thread-metrics-runtime`；context lifecycle/post-run compaction/SDK compact 状态已抽到 `context-lifecycle-service`；compaction archive/pending audit/ledger append 已抽到 `compaction-audit-service`；run-scoped billing state 已抽到 `active-run-billing-state`；run-scoped controller/worktree state 已抽到 `active-run-runtime-state`；run attempt lifecycle/retry/partial settlement 编排已抽到 `thread-run-attempt`；run outcome 判定已抽到 `thread-run-outcome`；run decision 公共副作用分派已抽到 `thread-run-decision-effects`；plan.ready 公共副作用已抽到 `thread-plan-ready-effects`；plan approval runtime gate 已抽到 `thread-plan-approval-runtime`；SDK task run hook extras 与 Stop 去重已抽到 `sdk-task-run-hooks`；run finally cleanup 已抽到 `thread-run-cleanup`；thread runtime route/proxy attempt 已抽到 `thread-runtime-routes` / `thread-runtime-proxy-attempt`；SDK run event loop 已抽到 `sdk-run-event-loop`；SDK usage recorded event handler/dispatch 已抽到 `sdk-usage-recorded-event-handler` / `sdk-usage-billing-dispatch`；SDK driver input 构造已抽到 `sdk-run-input`；SDK run 计费归因已抽到 `sdk-run-billing-attribution`；proxy 与 SDK event 前置 SubAgent 归因已抽到 `subagent-usage-attribution`；assistant fallback gating observation 去重已抽到 `usage-billing-observations`；OTel usage billing 解析已抽到 `otel-usage-billing`；Proxy usage billing 解析已抽到 `proxy-usage-billing`；SDK event usage 分流已抽到 `sdk-event-usage-billing`；SDK final run billing 编排已抽到 `sdk-run-billing-resolution`；single usage billing 编排已抽到 `single-usage-billing-orchestration`；stream partial billing 编排已抽到 `sdk-stream-partial-billing-orchestration`；billing runtime 依赖已收口到 `billing-runtime-environment`；账单快照选择已收口到 coordinator gate，并通过 `billing-snapshot-selection-policy` 默认请求 verified ledger projection；`index.ts` 仍保留 pricing cache/provider store 拥有权，下一批继续梳理 legacy fallback 删除路径。
- Pending plan IPC 读取视图已抽到 `thread-pending-plan-view`，审批启动门禁已抽到 `thread-plan-approval-runtime`，dismiss/cancel 前置决策已抽到 `thread-pending-plan-dismissal`；approve handler 仍持有状态更新和启动执行副作用，dismiss pending plan 仍在主进程持有运行时取消、pending 清理与 worktree finalizer 分派副作用，后续需要以完整行为切点继续拆分。
- TODO list IPC 回填已抽到 `thread-todo-list-runtime`；运行中 SDK TaskCreate/TaskUpdate/todo.updated tracker 装配已抽到 `thread-sdk-task-runtime`，底层行为仍由 `sdk-task-tracker` 和 `sdk-task-run-hooks` 驱动，主进程只负责装配 store 和 event emitter。
...
1. 梳理 legacy `recordSdkUsage` fallback 的真实调用面和 projection gate 条件，准备可审计删除路径。
2. 梳理 `index.ts` 剩余 pending plan continuation、approval launch side effects 与 dismissal finalizer 分派，寻找可测试的完整运行时切点。
3. 在更多真实路径对账稳定后，基于 `resolveSyntheticSdkPrimaryFill` 的 reason 逐步删除 legacy synthetic SDK primary 兼容写入，并继续收敛 `recordSdkUsage` fallback。
