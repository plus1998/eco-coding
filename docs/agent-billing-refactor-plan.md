...
- `bun test`: 通过，`812 pass / 14 skip / 0 fail`
...
1. 继续把 manual execution 和 continuation 的 run decision 副作用接入 `thread-run-decision-effects`，保留 todo stop / restore 顺序。
2. 继续收敛 `SubagentMetricsRegistry` 的 tool use linkage 与 legacy fallback 编排，准备删除 `recordSdkUsage` fallback。
