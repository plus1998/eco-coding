...
- **PI** uses [earendil-works/pi](https://github.com/earendil-works/pi) (`@earendil-works/pi-coding-agent`). supports Agent / Plan / Ask with built-in read/write/edit/bash (Ask/Plan also enable grep/find/ls; bash is limited to a read-only allowlist); Eco injects Skills and MCP per session (`.agents/skills` / `.pi/skills` plus thread-private `pi-agent/<threadId>/skills`; Composer-selected MCP / browser / image integrations via `pi-mcp-adapter`, withheld during Ask/Plan planning); session JSONL is stored under `userData/pi-agent/<threadId>/sessions/` (survives app restart; MCP/model-route changes start a fresh session); Agent mode can delegate Eco subagents from the thread orchestration snapshot; tool and plan approvals are Eco-bridged (BashApproval for tools; `finalize_plan` for plan cards). **Mobile has not verified PI-thread tool/plan approvals.**
