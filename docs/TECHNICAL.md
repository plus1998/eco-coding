...
**PI（v1）边界：** 进程内 `@earendil-works/pi-coding-agent` **0.85.1** SDK + Eco Gateway 模型；经 Eco Agent 工具接入线程编排快照中的子代理（capability=`eco`）；工具审批 capability=`eco`（`tool_call` → `createThreadToolPermissionHandler`）；会话模式 Agent/Plan/Ask；计划审批 capability=`eco`（`finalize_plan` → `plan.ready` 落盘 pending plan，回合结束，用户异步批准后新开 Agent 执行；退出不清 pending plan）；Ask/Plan 为只读工具 + 只读 bash allowlist（非 OS sandbox；`read` 经 Eco 工作区范围策略，区内放行、区外询问；只读 bash 仍由模式层 allowlist 放行）；**未接入** PI 可选 Windows `powershell` 工具（社区定位为无 Git Bash 时的可选替代，Ask/Plan 尚无 PS 只读策略）；**未接入** `SessionManager.inMemory(entries)` 外存恢复（主/子会话继续磁盘 JSONL）；**未开启** `compat.supportsMidConvoEffort`（pi 文档要求忠实 Anthropic Messages + 确切 Claude 模型；Eco Bridge/Gateway 不满足）；`ui_prompt_*` / `session_compact_failed` 扩展事件未单独桥接（审批走 Eco 工具权限；compaction 失败已由会话流 `compaction_end` 映射）。无 Eco compact/rewind handoff（compact 由 PI native `compaction.enabled` 负责，不装 pi-smart-compact；0.84.4+ 大工具结果会在同 turn 内先 compact 再请求）。Skills 与 MCP 由 Eco 按会话注入（`skills: "eco"` / `mcp: "eco"`）：Skills 发现 `.agents/skills` 与 `.pi/skills`（及 `~/.agents/skills` / `~/.pi/agent/skills`），经线程 `skillsEnabled` 过滤后传入 PI `ResourceLoader`（`includeDefaults: false`）。MCP 经 `pi-mcp-adapter` 的 in-memory `createMcpAdapter({ config })` 注入，只含 Composer 选中项与内置集成（浏览器/图片），不合并 ambient `.mcp.json`。设置「全局规则」经 `appendSystemPrompt` 注入父会话（先于浏览器/图片集成提示）；仅在创建/重建会话时写入 ResourceLoader，热复用不因规则变更刷新；子代理使用模板 prompt，不含全局规则。子代理为独立 `AgentSession`（`pi-agent/<threadId>/subagents/<agentId>/`，空 history，禁止嵌套），模型走该 agent 的 Gateway alias，不回落主模型。每线程 Eco 拥有目录 `ecoDataDir/pi-agent/<threadId>/`（私有 Skills 挂载、`sessions/*.jsonl` 持久化会话、auth/models 缓存）；**不**使用 `~/.pi` 默认会话目录。进程内 registry 热路径复用同一 `AgentSession`；冷启动在 identity + MCP fingerprint 匹配时 `SessionManager.open` 续跑。MCP 集合或 cwd/provider/model/apiCompat/routes 变更会重建空会话并清理该线程旧 jsonl。路由 `thinkingEffort` 映射为 PI `thinkingLevel`（`off`/`low`/`medium`/`high`/`xhigh`/`max` 1:1；未设置或未知值 fail-close 为 `off`）；非 `off` 时 `model.reasoning=true` 且 identity fingerprint 带 `:te=`，否则 PI 不发送思考字段、fingerprint 与旧会话兼容。Eco 没有 PI 的 `minimal` 档，不会发出该值。删线程会整树删除 `pi-agent/<threadId>`；存储面板「全部 PI 会话」会删除对应 Eco 对话（侧边栏不可再打开）并清理 `pi-agent/`（进行中的 PI 对话会跳过）。改 Skills 后 idle 会话可在下次 run 热更新（`AgentSession.reload`）；运行中仍禁止改配置。这是**可见性隔离**，不是 OS 级文件系统隔离——PI 仍有 `read`/`bash`，共享 workspace 内的 skill 文件其他会话进程仍可能读到。工具 cwd 以会话 `ctx.cwd` 为准（PI 0.85.0+；多会话同进程不再误用创建时 cwd）。

**PI 运行中跟进（steer）：** PI 线程在 run 运行中提交的后续消息经 `AgentSession.steer()` 注入当前回合（消息在下一个模型调用前投递，不新开一轮）；只有进程内父会话正在 streaming 时接受，会话空闲或 run 恰好在投递前结束时 `createPiMidTurnHandle` 会回收悬挂队列项并 `fail-close`，Eco 保留排队行、由 run 结束后的 drain 作为新一轮发送。子代理会话不参与线程级 steer。
