...
- **PI**：使用 [earendil-works/pi](https://github.com/earendil-works/pi) 的 coding-agent（`@earendil-works/pi-coding-agent`）。支持 Agent / Plan / Ask；内置 read/write/edit/bash（Ask/Plan 另开 grep/find/ls，bash 仅只读 allowlist）；Eco 按会话注入 Skills 与 MCP（`.agents/skills` / `.pi/skills` + 线程私有 `pi-agent/<threadId>/skills`；Composer 选中的 MCP / 浏览器 / 图片集成经 `pi-mcp-adapter` 注入，Ask/Plan 规划阶段不暴露 MCP）；会话 JSONL 持久化在 `userData/pi-agent/<threadId>/sessions/`（跨重启可续跑；改 MCP/模型路由会新开会话）；Agent 模式支持按会话编排快照委派子代理；工具审批与计划审批均由 Eco 桥接（工具确认同 Claude BashApproval；Plan 经 `finalize_plan` 走计划确认卡）。**Mobile 对 PI 线程工具/计划审批未验证**。
