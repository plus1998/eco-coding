-- Allow desktop (and any owned active device) to update public profile via
-- eco_update_device_profile. Previously restricted to kind = mobile only, so
-- renaming a registered desktop only updated local SQLite.
...
begin
  ...
  if not found then
    raise exception 'Active device not found for current user.' using errcode = '42501';
  end if;
