-- PostgREST treats custom SQLSTATE 40001 as a serialization failure and may retry the RPC indefinitely.
-- PT409 is the API conflict code; it preserves optimistic-concurrency semantics without retry loops.
-- This migration replaces the deployed function definition and keeps the provider-proxy/ssh whitelist.

-- Extend the account-config snapshot secret whitelist:
--   - 'provider-proxy'  new: per-provider outbound proxy URL (secret value keyed by provider id)
--   - 'ssh'             fix pre-existing gap: SSH bookmark password/key secrets are collected by
--     clients but rejected by this whitelist, making snapshot pushes fail for users with SSH credentials
...
begin
  ...
  if exists (
    ...
    where s.secret_kind not in ('provider', 'provider-proxy', 'asr', 'image', 'workflow', 'proxy', 'ssh')
  ...
  on conflict (user_id, secret_kind, secret_key) do update
