...
每个会话都可以选择 Codex、Claude Code 或 PI 作为 Agent Core。Codex / Claude 尽量保留各自官方运行方式与工具语义；**PI（v1）** 使用 `@earendil-works/pi-coding-agent` 进程内 SDK，经 Eco Gateway 调用模型，按会话注入 Skills 与 MCP，按会话编排快照委派 Eco 子代理；工具审批由 Eco 桥接（与 Claude 同一套确认卡与 `bashReviewMode`）；计划审批仍不支持。你可以在同一个项目中同时开跑不同内核的会话。
...
| 领域 | 能力 |
| --- | --- |
| Agent Core | Codex、Claude Code、PI；每个会话独立选择（PI 子代理由 Eco 按会话编排快照注入；工具审批由 Eco 桥接；计划审批仍不支持；Skills / MCP 由 Eco 按会话注入） |
