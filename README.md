...
每个会话都可以选择 Codex、Claude Code 或 PI 作为 Agent Core。Codex / Claude 尽量保留各自官方运行方式与工具语义。你可以在同一个项目中同时开跑不同内核的会话。
...
| 领域 | 能力 |
| --- | --- |
| Agent Core | Codex、Claude Code、PI；每个会话独立选择（PI 支持 Agent/Plan/Ask；子代理由 Eco 按会话编排快照注入；工具与计划审批由 Eco 桥接；Skills / MCP 由 Eco 按会话注入） |
